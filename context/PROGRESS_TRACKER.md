...
Last updated: 2026-06-25 — *PP-2: Call Emotion Heatmap widget added.*
...
| ID | Task | Status |
|---|---|---|
| PP-1 | Dashboard Overview page `/overview` | DONE |
| PP-2 | Call Emotion Heatmap widget (workforce page) | DONE |

---

## Changelog (newest first)
- **2026-06-25 (PP-2: Call Emotion Heatmap)** — New schema models: `CallRecording` (interactionId, audioUrl, durationSec, analyzedAt), `CallSegment` (recordingId, startSec, endSec, emotion EmotionLabel, intensity 0-1, label/labelAr), `EmotionLabel` enum (CALM/NEUTRAL/CONCERN/FRUSTRATION/ANGER/DISTRESS). Migration `20260625125627_add_call_heatmap` applied. `lib/queries/call-heatmap.ts` query with cluster+date filters. `components/widgets/call-heatmap/` — RSC shell + `CallHeatmapClient`: recording selector dropdown, mock HTML5 audio player with fake ticker playback, thermal heatmap bar (segments colored by emotion, intensity-modulated opacity, FRUSTRATION uses `color-mix` oklch blend), hover tooltips (emotion/intensity/time range), time axis labels, legend row, 3-col summary strip (worst emotion, calm %, stress %). Seed extended: 5 voice interactions get recordings with 6-8 segments each (5 emotion arcs). Widget mounted on `/workforce` page. 0 TS errors.
- **2026-06-21 (Overview page)** — PP-1: `/overview` page added. `lib/queries/overview.ts` (5 queries: getOverviewHeroStats, getOverviewSlaTrend, getOverviewAiResolution, getOverviewClusterHealth, getOverviewChannelVolume). 3 client components: `overview-hero-client.tsx` (OverviewHeroStats cards + ModuleQuickLinks), `overview-charts-client.tsx` (SlaTrendChart area, AiResolutionDonut, ChannelVolumeBar, ClusterHealthList), `overview-incidents-client.tsx` (OverviewIncidentsMini). RSC page with 5 Suspense+ErrorBoundary rows: hero stats, SLA trend + AI donut (2-col), incidents + cluster health (2-col), channel bar, quick links. Overview added to sidebar (LayoutDashboard icon, noStatusDot, first item). Status dot guard added for mobile nav. 0 TS errors.
