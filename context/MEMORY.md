...
## Change Log
- **2026-06-21 (Phase 5 DONE)** — 5 Governance widgets. Key decisions: (1) shadcn canary `Button` has no `asChild` prop (base-ui) — replaced all Button-wrapping-anchor patterns with plain `<a>` styled via Tailwind classes. (2) `filtersToParams()` in `lib/filters.ts` returns a string (not `URLSearchParams`) with leading `?` — export URL built with local `buildExportQs()` helpers instead. (3) Export route at `app/api/export/[kind]/route.ts` streams CSV, COMPLIANCE-gated server-side. (4) KnowledgeBase publish action increments `version` on every publish. (5) ForbiddenIntent chart click uses Recharts `onClick` on `AreaChart` — `activeLabel` carries the formatted date string, matched against raw ISO date in event list. (6) ConsentAudit deep-link passes `?highlight=<caregiverCaseId>` to `/intelligence` for future highlight support.
